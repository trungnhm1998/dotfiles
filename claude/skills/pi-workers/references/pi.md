...
- **No MCP client** (verified v0.84.1). No `mcp__*` tools exist in a pi session, ever.
  Route MCP-bound tasks (Unity Editor writes, browser MCP, etc.) to a built-in Claude
  subagent. A correctly written dispatch makes pi fail loudly: it reports
...
- **Instructions**: pi concatenates `~/.pi/agent/AGENTS.md` + parent-dir + cwd `AGENTS.md`
  (and honours `CLAUDE.md`), and auto-loads repo `.agents/skills/`. Repo conventions and
  skills arrive without you restating them in the dispatch.
...
send the one-line dispatch pointer. Six workers ran this way with zero shell-parse
failures because nothing long ever crosses the command line.
...
- **PowerShell parses your command first.** `@"..."` is a here-string header in
  ...
  `'@path/dispatch.md'`.
- **Silent until done.** `pi -p` prints nothing while running. The only live window is
  the session log: `~/.pi/agent/sessions/<cwd-slug>/<timestamp>_<session-id>.jsonl` —
  one JSONL record per thinking block and tool call. `LastWriteTime` rising = alive;
  last line names the tool call it is inside.
...
- **Provider death looks like laziness.** pi's default provider can start returning
  instant empty responses mid-pipeline: exit in seconds, `totalTokens: 0`, no content,
