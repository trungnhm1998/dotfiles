...
| `.config/profile` | `$HOME\.config\profile` (gaming/work profile toggle — `profile-toggle.ps1` engine + `$Apps` table; entry points: pwsh `game`/`game -Off` (alias `ungame`, restores the pre-game snapshot at `~/.config/dotfiles/profile-snapshot.json`)/`work`, YASB pill, AHK service-mode `g`, Raycast. Marker `~/.config/dotfiles/profile` drives the `dotfiles-profile-boot` logon task; elevated ops via `dotfiles-profile-elevated`. Design: `docs/specs/2026-07-13-gaming-profile-design.md` + `docs/specs/2026-09-02-gaming-session-snapshot-design.md`) |
...
| Profile toggle (Windows) | `.config/profile/profile-toggle.ps1` (engine; `game`/`ungame`/`work` in pwsh) · `.config/profile/profile-elevated.ps1` (elevated task body) |
