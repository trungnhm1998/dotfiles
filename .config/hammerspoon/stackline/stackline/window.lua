...
function Window:getColorAttrs(isStackFocused, isWinFocused) -- {{{
    ...
    -- Get app-specific color or fall back to default
    local appColors = opts.appColors or {}
    local baseColor = appColors[self.app] or opts.color
    ...
    local colorLookup = {
        stack = {
            ['true'] = {
                window = {
                    ['true'] = {
                        bg = u.extend(baseColor, {alpha = opts.alpha}),
                        img = opts.alpha,
                    },
                    ['false'] = {
                        bg = u.extend(u.copy(baseColor),
