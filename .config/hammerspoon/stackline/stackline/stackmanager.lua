...
function Stackmanager:init() -- {{{
    self.tabStacks = {}
    self.spaceCache = {}  -- cache stacks per space: { [spaceId] = { stacks = {...}, windowIds = {...} } }
...
function Stackmanager:getCurrentSpaceId() -- {{{
    ...
    -- Fallback: get active space for main screen
...
function Stackmanager:cacheCurrentSpace() -- {{{
    local spaceId = self:getCurrentSpaceId()
    if not spaceId then return end

    -- Get window IDs for comparison later
    local windowIds = {}
    for _, stack in pairs(self.tabStacks) do
        for _, win in pairs(stack.windows) do
            table.insert(windowIds, win.id)
        end
    end
    table.sort(windowIds)

    self.spaceCache[spaceId] = {
        stacks = self.tabStacks,
        windowIds = windowIds
    }
    self.currentSpaceId = spaceId
    log.i('Cached space:', spaceId, 'with', #self.tabStacks, 'stacks')
end -- }}}

function Stackmanager:hideCurrentSpace() -- {{{
    -- Hide indicators for current space (for caching)
    self:eachStack(function(stack)
        stack:hideAllIndicators()
    end)
end -- }}}

function Stackmanager:showCachedSpace(spaceId) -- {{{
    local cached = self.spaceCache[spaceId]
    if not cached then return false end

    self.tabStacks = cached.stacks
    self:eachStack(function(stack)
        stack:showAllIndicators()
    end)
    self.currentSpaceId = spaceId
    log.i('Restored cached space:', spaceId)
    return true
end -- }}}

function Stackmanager:hasCacheForSpace(spaceId) -- {{{
    return self.spaceCache[spaceId] ~= nil
end -- }}}

function Stackmanager:isCacheValid(spaceId, currentWindowIds) -- {{{
    local cached = self.spaceCache[spaceId]
    if not cached then return false end

    -- Compare window IDs to see if cache is still valid
    table.sort(currentWindowIds)
    if #cached.windowIds ~= #currentWindowIds then
        return false
    end
    for i, id in ipairs(cached.windowIds) do
        if id ~= currentWindowIds[i] then
            return false
        end
    end
    return true
...
function Stackmanager:invalidateCache(spaceId) -- {{{
    if self.spaceCache[spaceId] then
        -- Delete indicators before removing from cache
        for _, stack in pairs(self.spaceCache[spaceId].stacks) do
            stack:deleteAllIndicators()
        end
        self.spaceCache[spaceId] = nil
        log.i('Invalidated cache for space:', spaceId)
    end
end -- }}}

function Stackmanager:cleanupAllCaches() -- {{{
    for spaceId, cached in pairs(self.spaceCache) do
        for _, stack in pairs(cached.stacks) do
            stack:deleteAllIndicators()
        end
    end
    self.spaceCache = {}
...
function Stackmanager:ingest(stacks, appWins, shouldClean) -- {{{
    ...
    -- Cache the current space after ingesting
    self:cacheCurrentSpace()
end -- }}}
