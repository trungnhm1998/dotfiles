...
stackline.spaceWatcher = hs.spaces.watcher.new( -- {{{
    function(spaceIdx)
        ...
        -- Hide current space indicators before switching (preserve in cache)
        stackline.manager:hideCurrentSpace()

        -- Get the new space ID
        local newSpaceId = stackline.manager:getCurrentSpaceId()

        -- Check if we have a valid cache for the new space
        if newSpaceId and stackline.manager:hasCacheForSpace(newSpaceId) then
            -- Try to show cached indicators
            log.i('Attempting to restore cached space:', newSpaceId)
            if stackline.manager:showCachedSpace(newSpaceId) then
                log.i('Successfully restored cached space')
        ...
        log.i('No cache for space, doing full refresh')
        stackline.forceRedraw = true
